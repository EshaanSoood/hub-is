...
import { tableContract } from './widgetPickerPreviewContracts';

const WidgetPickerHarness = () => {
...
describe('WidgetPickerOverlay', () => {
  it('uses viewport-fixed overlay and panel classes', async () => {
    ...
    const overlay = document.querySelector('.widget-picker-viewport-backdrop');
    const panel = await screen.findByRole('dialog', { name: 'Add Widget' });

    expect(overlay).toHaveClass('fixed', 'inset-0', 'widget-picker-viewport-backdrop');
    expect(panel).toHaveClass('fixed', 'widget-picker-viewport-panel', 'widget-picker-panel-size');
  });

  it('traps keyboard focus inside the picker and restores focus to the trigger', async () => {
    mockVisibleElements();
    const user = userEvent.setup();
    const { container } = render(<WidgetPickerHarness />);
