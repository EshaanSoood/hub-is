...
import { useWidgetInsertState } from '../hooks/useWidgetInsertState';
...
import { UNASSIGNED_ID, type KanbanWidgetSkinProps } from './types';
...
export const KanbanWidgetSkin = ({
  ...
  onDeleteRecord,
  onInsertToEditor,
}: KanbanWidgetSkinProps) => {
  const {
    activeItemId,
    activeItemType,
    setActiveItem,
    clearActiveItem,
  } = useWidgetInsertState({ onInsertToEditor });
  ...
  if (loading) {
    return <WidgetLoadingState label="Loading kanban cards" rows={6} />;
  }
