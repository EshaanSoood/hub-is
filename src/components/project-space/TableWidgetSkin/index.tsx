...
import type { TableField, TableWidgetSkinProps, TableRowData } from './types';
import {
  formatDisplayValue,
...
export const TableWidgetSkin = ({
  ...
  const getScrollElement = useCallback(() => {
    ...
    return (node.closest('[data-widget-card-body="true"]') as HTMLDivElement | null) ?? node;
  }, []);
  ...
  if (loading) {
    return <WidgetLoadingState label="Loading table records" rows={6} />;
  }
  ...
  if (!schema) {
    ...
    if (!canSelectView) {
      return <WidgetEmptyState title="No table view found yet." iconName="table" description="Table For Two?" sizeTier={sizeTier} />;
    }
  ...
  return (
    <section className={cn('widget-sheet flex min-h-0 flex-col', previewMode ? 'w-full' : 'h-full')} aria-label="Table widget">
      <TableHeader
        table={table}
