...
import type { KanbanWidgetContract } from '../widgetContracts';

const KanbanWidgetSkin = lazy(async () => {
  const module = await import('../KanbanWidgetSkin');
  return { default: module.KanbanWidgetSkin };
});

interface Props {
  widget: ContractWidgetConfig;
  contract: KanbanWidgetContract;
...
const resolveBoundViewId = (
  widget: ContractWidgetConfig,
  views: KanbanWidgetContract['views'],
  ...
  const ownedViewId = widget.binding?.owned_view_id;
  if (ownedViewId && views.some((view) => view.view_id === ownedViewId)) {
    return ownedViewId;
  }
  if (widget.binding?.source_mode === 'owned') {
    return null;
  }
...
export const KanbanWidget = ({
  ...
  const isOwnedMode = widget.binding?.source_mode === 'owned';
  const ownedViewId = widget.binding?.owned_view_id ?? null;
  const isCreatingView = Boolean(contract.creatingViewByWidgetId?.[widget.widget_instance_id]);
  ...
  const handleEnsureView = useCallback(async () => {
    ...
    setSetupError(null);
    try {
      const viewId = await contract.onEnsureView(widget.widget_instance_id, ownedViewId);
      if (viewId) {
        onSetWidgetBinding(widget.widget_instance_id, {
          ...widget.binding,
          source_mode: 'owned',
  ...
  }, [acquireAutoEnsureLock, contract, widget, onSetWidgetBinding, ownedViewId]);
  ...
  if (needsStandaloneBoard) {
    if (isCreatingView) {
      return <WidgetLoadingState label="Preparing kanban widget" visibleLabel="Preparing kanban board" rows={4} />;
    }
      ...
      {contract.views.length > 0 && !previewMode ? (
        <label className="block text-xs text-muted">
          ...
          <select
            ...
            onChange={(event) => onSetWidgetBinding(widget.widget_instance_id, {
              ...widget.binding,
