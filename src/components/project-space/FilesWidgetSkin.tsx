...
import { useWidgetInsertState, type WidgetInsertPayload, type WidgetInsertState } from './hooks/useWidgetInsertState';
import { FileRecordSummary } from './record-primitives/FileRecordSummary';
import { Icon } from '../primitives';

export interface FilesWidgetItem {
  id: string;
  name: string;
  ext: string;
  sizeLabel: string;
...
interface FilesWidgetSkinProps {
  sizeTier: 'S' | 'M' | 'L';
  files: FilesWidgetItem[];
...
const normalizeExt = (file: FilesWidgetItem): string => file.ext.toLowerCase();

const filterFiles = (files: FilesWidgetItem[], filter: FilterKey): FilesWidgetItem[] => {
  if (filter === 'all') {
    return files;
  }
  if (filter === 'images') {
...
const uploadLabel = (file: FilesWidgetItem): string => {
...
const DropZone = ({
  ...
  const containerClassName = cn(
    'widget-dropzone relative transition-colors',
...
const FileRow = ({
  ...
  file: FilesWidgetItem;
  onOpen: (file: FilesWidgetItem) => void;
...
const FilesWidgetSmall = ({
  ...
  return (
    <div className="widget-sheet flex h-full min-h-0 flex-col gap-xs p-sm">
...
const FilesWidgetMedium = ({
  ...
  return (
    <div className="widget-sheet flex h-full min-h-0 flex-col gap-sm p-md">
...
const FilesWidgetLarge = ({
  ...
  return (
    <div className="widget-sheet flex h-full min-h-0 flex-col gap-md p-md">
...
export const FilesWidgetSkin = ({
  ...
  return (
    <section className="flex h-full min-h-0 flex-col gap-2" aria-label="Files widget">
      <p className="sr-only" aria-live="polite">
        {liveMessage}
      </p>
      {sizeTier === 'S' ? <FilesWidgetSmall files={files} onUpload={onUpload} onOpenFile={onOpenFile} insertState={insertState} readOnly={readOnly} previewMode={previewMode} /> : null}
      {sizeTier === 'M' ? <FilesWidgetMedium files={files} onUpload={onUpload} onOpenFile={onOpenFile} insertState={insertState} readOnly={readOnly} previewMode={previewMode} /> : null}
      {sizeTier === 'L' ? <FilesWidgetLarge files={files} onUpload={onUpload} onOpenFile={onOpenFile} insertState={insertState} readOnly={readOnly} previewMode={previewMode} /> : null}
    </section>
  );
};
