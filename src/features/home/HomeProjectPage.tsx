...
import { HomeShell, suppressNextHomeHeadingFocus } from './HomeShell';
...
export const HomeProjectPage = () => {
  ...
  const quickThoughtsTriggerRef = useRef<HTMLElement | null>(null);
  const [autoFocusHeading, setAutoFocusHeading] = useState(true);
  ...
  const openQuickThoughts = useCallback(() => {
    setAutoFocusHeading(false);
    quickThoughtsTriggerRef.current = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    setSearchParams((current) => {
      const next = new URLSearchParams(current);
      next.set('surface', 'thoughts');
      return next;
    }, { replace: true });
  }, [setSearchParams]);

  const closeQuickThoughts = useCallback((options?: { restoreFocus?: boolean }) => {
    suppressNextHomeHeadingFocus();
    setSearchParams((current) => {
      const next = new URLSearchParams(current);
      next.delete('surface');
      return next;
    ...
    window.requestAnimationFrame(() => {
      if (quickThoughtsTriggerRef.current?.isConnected) {
        quickThoughtsTriggerRef.current.focus();
        return;
      }
      ...
      <HomeShell
        ...
        overviewContent={(
          <HomeDashboardSurface
            ...
            homeLoading={homeRuntime.homeLoading}
            onOpenQuickThoughts={openQuickThoughts}
            onOpenRecord={openRecord}
