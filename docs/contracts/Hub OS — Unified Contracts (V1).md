...
- `project_id` (FK projects)
- `collection_id` (FK collections)
- `type` (`table | kanban | list | calendar | timeline | gallery`)  
  - V1: `table`, `kanban` required; others optional later
- `name`
- `config` (json)
  - visible fields
  - filters/sorts
...
#### `recurrence_rules`
- `record_id` (PK, FK records)
- `rule_json` (json) — compatible with NLP output (frequency/days/end_date/etc.)
- `updated_at`
...
#### Attachments (Hub references, Nextcloud-backed in V1 web phase)
##### `files`
- `file_id` (PK)
- `project_id` (FK projects)
- `asset_root_id` (FK asset_roots)
- `provider` (`nextcloud`)
- `provider_path` (path relative to asset root)
- `name`
- `mime_type`
- `size_bytes`
- `hash` (optional)
...
- `provider`
- `asset_root_id` (FK asset_roots)
- `asset_path` (provider path relative to root)
- `name`, `mime_type`, `size_bytes`
- `metadata_json`
- `created_by` (FK users)
- `created_at`

> **V1 locked decision:** Nextcloud is the project hard drive source of truth.  
> Uploads land in Nextcloud under the project `asset_root` (default subfolder such as `/Uploads`).  
> Attachments persist asset references (provider/path/metadata), not separate binary blob ownership in Hub.
...
> V1: **No local cache**. Browser reads via Nextcloud API through backend proxy.
...
#### `comment_anchors` (doc node anchors)
- `comment_id` (PK, FK comments)
- `doc_id` (FK docs)
- `anchor_payload` (json) — **V1 shape:** `{ kind: "node", nodeKey, ...optional_context }`
- `created_at`, `updated_at`

> **V1 locked decision:** doc comments are anchored to Lexical nodes (`nodeKey`), not absolute text offsets/ranges.  
> Anchors must survive move/reorder.  
> If a referenced node is deleted, the comment remains retrievable and is marked **orphaned** in doc comment queries.
...
- doc comments anchored to Lexical nodes in `comment_anchors` (`kind: "node"`, `nodeKey`), with orphaned-thread surfacing when node is removed
...
### 4.8 Attachments
#### `UploadFileResponse`
- `file`: { `file_id`, `asset_root_id`, `provider`, `asset_path`, `name`, `mime_type`, `size_bytes` }

#### `AttachFileRequest`
- `project_id`
- `entity`: EntityRef
- `asset_ref`: { `provider`, `asset_root_id`, `asset_path`, `name`, `mime_type`, `size_bytes`, `metadata` }
...
- `anchor` (optional):
  - `doc_id`
  - `anchor_payload` (`{ kind: "node", nodeKey, ... }` in V1)
