...
export const seedModulePickerSeedData = (db, nowIso) => {
  ...
  for (const [moduleType, sizes] of Object.entries(allowedSizesByModule)) {
    for (const size of sizes) {
      insertSeed.run(moduleType, size, JSON.stringify(baseSeeds[moduleType]), now, now);
    }
  }
};
